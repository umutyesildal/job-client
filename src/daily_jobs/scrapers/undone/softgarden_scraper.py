"""
Softgarden Job Scraper
Scrapes job listings from Softgarden ATS platform
"""
...
class SoftgardenScraper:
    """Scraper for Softgarden job boards"""
    ...
    def scrape_jobs(self, url: str, company_name: str = "", company_description: str = "", label: str = "") -> List[Dict]:
        """Scrape all jobs from Softgarden job board"""
        logger.info(f"Scraping Softgarden job board: {url}")
        jobs = []
        ...
        try:
            ...
            job_items = soup.find_all(['a', 'div', 'li'], class_=re.compile(r'job|position|stelle', re.I))
            ...
            logger.error(f"Error scraping Softgarden board: {e}")
    ...
    def _extract_job_data(self, item, base_url: str, company_name: str, company_description: str, label: str) -> Optional[Dict]:
        ...
        title = item.find(['h2', 'h3', 'h4', 'span'], class_=re.compile(r'title|name|titel', re.I))
        if title:
        ...
        location = item.find(['span', 'div'], class_=re.compile(r'location|ort|standort', re.I))
        if location:
            job['Location'] = location.get_text(strip=True)
        
        dept = item.find(['span', 'div'], class_=re.compile(r'department|team|bereich', re.I))
        if dept:
            job['Department'] = dept.get_text(strip=True)
        
        return job
    
    def save_to_csv(self, jobs: List[Dict], output_file: str = 'softgarden_jobs.csv'):
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Scrape jobs from Softgarden job board')
    parser.add_argument('url', help='Softgarden job board URL')
    ...
    parser.add_argument('-o', '--output', default='softgarden_jobs.csv', help='Output CSV file')
