"""
Join Job Scraper
Scrapes job listings from Join ATS platform
"""
...
class JoinScraper:
    ...
    def scrape_jobs(self, url: str, company_name: str = "", company_description: str = "", label: str = "") -> List[Dict]:
        """Scrape all jobs from Join job board"""
        logger.info(f"Scraping Join job board: {url}")
        jobs = []
        ...
        try:
            ...
            job_items = soup.find_all(['a', 'div'], class_=re.compile(r'job|position|vacancy', re.I))
            ...
            logger.error(f"Error scraping Join board: {e}")
    ...
    def save_to_csv(self, jobs: List[Dict], output_file: str = 'join_jobs.csv'):
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Scrape jobs from Join job board')
    parser.add_argument('url', help='Join job board URL')
    ...
    parser.add_argument('-o', '--output', default='join_jobs.csv', help='Output CSV file')
