"""
Gem Job Scraper
Scrapes job listings from Gem ATS platform
"""
...
class GemScraper:
    ...
    def scrape_jobs(self, url: str, company_name: str = "", company_description: str = "", label: str = "") -> List[Dict]:
        """Scrape all jobs from Gem job board"""
        logger.info(f"Scraping Gem job board: {url}")
        jobs = []
        ...
        try:
            ...
            # Gem uses specific patterns for job listings
            job_items = soup.find_all(['a', 'div'], class_=re.compile(r'job|position|role', re.I))
            ...
            logger.error(f"Error scraping Gem board: {e}")
    ...
    def save_to_csv(self, jobs: List[Dict], output_file: str = 'gem_jobs.csv'):
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Scrape jobs from Gem job board')
    parser.add_argument('url', help='Gem job board URL')
    ...
    parser.add_argument('-o', '--output', default='gem_jobs.csv', help='Output CSV file')
